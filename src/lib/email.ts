...
function getFrom(): string {
  return (
    ...
    "Electroplace <noreply@electroplace.co.uk>"
  );
}
...
function getSiteUrl(): string {
  const raw = process.env.NEXT_PUBLIC_SITE_URL || "https://electroplace.co.uk";
  return raw.replace(/\/+$/, "");
}
  ...
  <div style="max-width:600px;margin:0 auto;padding:32px 16px;">
    <div style="text-align:center;margin-bottom:24px;">
      <a href="${getSiteUrl()}" style="text-decoration:none;">
        <span style="font-size:24px;font-weight:900;color:${BRAND_COLOR};letter-spacing:-0.03em;">Electroplace</span>
      </a>
    </div>
    ...
    <div style="text-align:center;margin-top:24px;font-size:12px;color:#999;line-height:1.6;">
      <p style="margin:0 0 4px;">&copy; ${new Date().getFullYear()} Electroplace. All rights reserved.</p>
      <p style="margin:0;">RYE FLOUR COOKIES LTD &middot; Bridgend, Mid Glamorgan, Academy House, 11 Dunraven Place &middot; Reg. 16020956 &middot; <a href="${getSiteUrl()}" style="color:${BRAND_COLOR};text-decoration:none;">electroplace.co.uk</a></p>
...
export async function sendWelcomeEmail(email: string, name?: string | null): Promise<boolean> {
  ...
  return send({
    to: email,
    subject: "Welcome to Electroplace!",
    html: emailWrapper(
      `
      <h1 style="margin:0 0 16px;font-size:22px;font-weight:800;color:${TEXT_COLOR};">${greeting}, welcome to Electroplace!</h1>
    ...
    `,
      { preheader: `${greeting}! Your Electroplace account is ready.` },
    ),
  });
}
...
export async function sendPasswordResetEmail(email: string, resetUrl: string, name?: string | null): Promise<boolean> {
  ...
  return send({
    to: email,
    subject: "Reset your Electroplace password",
    html: emailWrapper(
      `
      <h1 style="margin:0 0 16px;font-size:22px;font-weight:800;color:${TEXT_COLOR};">Reset your password</h1>
      <p style="color:${MUTED_COLOR};line-height:1.6;margin:0 0 16px;">
        ${greeting}, we received a request to reset the password on your Electroplace account.
    ...
    `,
      { preheader: "Reset your Electroplace password — link expires in 1 hour." },
    ),
  });
}
...
export async function sendContactAutoReplyEmail(submission: ContactSubmission): Promise<boolean> {
  return send({
    to: submission.email,
    subject: "We received your message — Electroplace",
    html: emailWrapper(
      `
